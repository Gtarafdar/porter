...
import { PORTER_KEEPALIVE_REV, PORTER_CORE_REV, writeStartPorterScript } from "../keepalive.js";
...
test("writeStartPorterScript embeds patient health + keepalive rev", () => {
  ...
  try {
    ...
    assert.match(body, new RegExp(`WANT_CORE="${PORTER_CORE_REV}"`));
    assert.match(body, /open-tailscale-app/);
